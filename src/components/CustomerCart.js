...
const CustomerCart = () => {
	...
	const getCart = async () => {
		const res = await axios.get(
			`https://bhoj-server.herokuapp.com/api/carts/find/${userId}`,
			{ headers: { token: token } },
		);
		setCart(res.data);
		if (res.data) {
	...
	const deleteCart = async (id) => {
		try {
			if (token) {
				await axios.delete(
					`https://bhoj-server.herokuapp.com/api/carts/${id}`,
					{
						headers: { token: token },
					},
				);
	...
	const handlePlusClick = async (itemId) => {
		...
		const updatedCart = await axios.put(
			`https://bhoj-server.herokuapp.com/api/carts/${cart._id}`,
			cart,
			{ headers: { token: token } },
		);
