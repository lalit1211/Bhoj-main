...
const GenerateReport = () => {
	...
	const getOrders = async () => {
		const res = await axios.get(
			"https://bhoj-server.herokuapp.com/api/orders/",
			{ headers: { token: token } },
		);
		setOrders(res.data);
	...
	const getSales = async () => {
		const res = await axios.get(
			"https://bhoj-server.herokuapp.com/api/orders/income",
			{ headers: { token: token } },
		);
		sales.push(res.data);
		...
		for (let i = 0; i < sales[0].length; i++) {
			if (sales[0][i]._id === currentMonth) {
				setCurrentIncome(
					(currentIncome) =>
						currentIncome + sales[0][i].total,
				);
			}
			if (sales[0][i]._id === prevMonth) {
				setPrevIncome(
					(prevIncome) =>
						prevIncome + sales[0][i].total,
				);
			}
		}
	...
	return (
		<div className='reports-page'>
			...
			<br />
			<h3>
				Total Income Generated (Lifetime) : INR{" "}
				{totalIncome()}
			</h3>
			<br />
			...
			<br />
			<h3>
				Income Generated in the Current Month = INR{" "}
				{currentIncome}
			</h3>
			<br />
			<h3>
				Income Generated in the Last Month = INR{" "}
				{prevIncome}
			</h3>
			<br />
			{parseFloat(
