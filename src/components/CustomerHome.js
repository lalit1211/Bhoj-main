...
const Home = () => {
	...
	const getItems = async () => {
		const res = await axios.get(
			"https://bhoj-server.herokuapp.com/api/items",
		);
		setItems(res.data);
	};
	...
	const addToCart = async (item) => {
		const getCart = await axios.get(
			`https://bhoj-server.herokuapp.com/api/carts/find/${id}`,
			{ headers: { token: token } },
		);
		...
		if (!getCart.data) {
			...
			const newCart = await axios.post(
				"https://bhoj-server.herokuapp.com/api/carts/",
				cartBody,
				{ headers: { token: token } },
			);
		...
		if (getCart.data) {
			...
			for (
				let i = 0;
				i < getCart.data.items.length;
				i++
			) {
				// if item found, increase the quantity by 1
				if (
					item._id ===
					getCart.data.items[i].itemId
				) {
					getCart.data.items[i].quantity += 1;
					const updatedCart = await axios.put(
						`https://bhoj-server.herokuapp.com/api/carts/${getCart.data._id}`,
						getCart.data,
						{ headers: { token: token } },
					);
